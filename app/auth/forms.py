...
class LoginForm(FlaskForm):
    email = StringField(
        'E-mail',
        validators=[
            DataRequired('Необходимо заполнить поле'),
            Length(1, 64),
...
class RegForm(FlaskForm):
    ...
    fname = StringField(
        'Введите ваше имя, которое будет видно другим пользователям*',
        validators=[
            DataRequired('Необходимо заполнить поле'),
            Length(2, 64),
            Regexp(
    ...
    lname = StringField(
        ...
        validators=[Optional(), Length(2, 64), Regexp(
            '^[А-Яа-я]+$',
            0,
    ...
    password = PasswordField(
        'Введите пароль*',
        validators=[DataRequired('Необходимо заполнить поле'), Length(8, 64)],
        render_kw={'class': 'form-control'},
    )
